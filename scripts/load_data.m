...
[results.pred, results.cmap] = imread(fullfile(opts.pred_root, opts.pred_dir, sprintf(opts.pred_template, results.id)));
temp = load(fullfile(opts.pred_root, opts.pred_score_dir, sprintf(opts.pred_score_template, results.id)));
...
results.gt_label = imread(fullfile(opts.data_root, opts.gt_label_dir, city, sprintf(opts.gt_label_template, results.id)));
results.mandg_pred = imread(fullfile(opts.pred_root, opts.mcg_and_grabcut_dir, sprintf(opts.mcg_and_grabcut_template, results.id)));
